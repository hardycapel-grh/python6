"""Autogenerated module docstring for access\ui\pages\admin\store_locations_page.py

Description: TODO - add a meaningful module description.
Generated: 2026-08-01T21:47:23.705232Z
"""
...
class StoreLocationsPage(QWidget):
    ...
    def load_locations(self):
        ...
        for loc in locations:
            ...
            # Store the full document in the first column's user-data so
            # actions (edit/delete/toggle) can access the original DB doc
            # including the _id without additional lookups.
            self.table.setRowHeight(row, 28)
            self.table.item(row, 0).setData(Qt.UserRole, loc)

        # CAUTION: We store the full DB document in the UI so edit/delete
        # handlers can access `_id` and other metadata without extra queries.
        # Treat the stored object as read-only to avoid accidental in-memory
        # mutations that are not persisted back to the DB.
    ...
    def delete_location(self):
        ...
        # Delete the document and write an audit entry describing the
        # change so administrators can trace configuration updates.
        ...
        # Refresh UI after mutation to reflect the current DB state.
        self.load_locations()
