"""Autogenerated module docstring for access/ui/pages/inventory/batch_list_page.py

Description: TODO - add a meaningful module description.
Generated: 2026-08-01T21:47:24.024655Z
"""
...
class BatchTableModel(QAbstractTableModel):
    ...
    def _expiry_status(self, value):
        ...
        # Interpret expiry relative to today to decide UI colouring/status.
        if dt < today:
            return "expired"
    ...
    def _days_until_expiry(self, value):
        ...
        # If parsing fails, return a sentinel string used in the UI table.
        if not dt:
            return "Invalid"
    ...
    def _days_until_expiry_numeric(self, value):
        ...
        # Invalid dates are treated as very far in the future so they sort
        # to the bottom when numeric sorting is applied.
        if not dt:
...
class EditBatchDialog(QDialog):
    ...
    def _parse_date(self, value):
        ...
        # Try common date formats in order of preference. The system accepts
        # ISO (`YYYY-MM-DD`) first, then common European and alternative
        # separators. This lenient parsing allows older data with mixed
        # formats to display correctly; if parsing fails the caller treats
        # the value as invalid and the UI highlights/sorts it accordingly.
        for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%Y/%m/%d"):
            try:
                from datetime import datetime
                dt = datetime.strptime(value, fmt)
...
class BatchListPage(QWidget):
    def __init__(self, mongo, item, user=None, parent=None):
        ...
        self.setWindowTitle(f"Batches a {item['part_number']}")
        self.setMinimumSize(600, 400)
        self.resize(900, 500)

        layout = QVBoxLayout(self)

        title = QLabel(f"Batches for {item['part_number']} Rev {item.get('revision', '')} a {item['description']}")
    ...
    def _adjust_stock(self, batch):
        ...
        performed_by = self.user.username if self.user else "system"
        # Audit stock adjustment: include both old and new quantities and a
        # reason so operators can trace inventory movements. Sensitive data
        # (none expected here) will be sanitized by the service.
        self.mongo.audit(
            event="stock.adjust",
