"""Autogenerated module docstring for access/ui/pages/inventory/edit_item_dialog.py

Description: TODO - add a meaningful module description.
Generated: 2026-08-01T21:47:24.064277Z
"""
...
class EditItemDialog(QDialog):
    def __init__(self, mongo, user, item, parent=None):
        ...
        self.setWindowTitle(f"Edit Item a {item.get('part_number')}")
    ...
    def _save(self):
        ...
        for key, new_value in self.updated.items():
            ...
            if old_value != new_value:
                changes.append(f"{key}: '{old_value}' a '{new_value}'")

        # Audit log
        # NOTE: This dialog records an audit event describing the intended
        # changes. The actual persistence (update_one) is performed by the
        # caller that created this dialog a be aware that writing the audit
        # before the DB update can create a mismatch if the subsequent write
        # fails. Prefer writing audits after successful persistence at the
        # service layer when possible.
        if changes:
            self.mongo.log_event(
                "inventory.edit",
                performed_by=self.user.username,
                details=f"Edited item {self.item.get('part_number')} a Changes: {'; '.join(changes)}"
            )
