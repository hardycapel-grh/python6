"""Autogenerated module docstring for access/ui/pages/inventory/stock_movement_dialog.py

Description: TODO - add a meaningful module description.
Generated: 2026-08-01T21:47:24.142836Z
"""
...
class StockMovementDialog(QDialog):
    def __init__(self, mongo, user, item, parent=None):
        ...
        self.setWindowTitle(f"Move Stock a {item['part_number']}")
    ...
    def _save(self):
        ...
        # NOTE: This movement uses a simple FIFO algorithm by `received_date`.
        # The operation is not wrapped in a DB transaction here, so partial
        # failures can leave source batches decremented without all destination
        # batches created. In production consider using multi-document
        # transactions (MongoDB replica set) or an application-level
        # compensation step to guarantee atomicity.
        ...
        for batch in batches:
            ...
            new_batch = {
                ...
                # a Copy cost + expiry from source batch
            ...
            # Each created MOVE batch is prefixed so auditors can trace it back
            # to the original source batch. Cost and expiry are explicitly copied
            # to preserve financial and shelf-life attributes across the move.
        ...
        # Audit: the `inventory.move` document captures the high-level move
        # intent and actor. The MongoService audit pipeline will sanitize
        # fields before writing to DB; however, consider attaching the
        # per-batch movement details if you need full traceability in the
        # audit collection.
